...
import contextsMenu from './transformers/contexts-menu';
import contextsKeys from './transformers/contexts-keys';
import applyMenuKeys from './transformers/apply-menu-keys';
import applyMenuConditions from './transformers/apply-menu-conditions';
import CONTEXT_CLASS from './constants/context-class';
import { contextTriggerAction, UNHANDLED } from './handle/action';
...
const initialiseContextSystem = (rootElement: HTMLElement): ContextSystemApi => {
  ...
  const decideMenuConfig = (id: ContextId, event: ContextEvent): ContextMenuItemListFilled => {
    ...
    const acts = contextsActs(contexts, { path, type, data, event });
    const keys = contextsKeys(contexts, { path, type, data, event });

    return applyMenuKeys(applyMenuConditions(menu, acts, { path, type, data, event }), keys, {
      path,
      type,
      data,
      event,
